...
BATCH_SIZE = 10
roc_curve_thresholds = np.linspace(0, 2, num=100)
...
def main():
    ...
    train(args.train_path, args.val_path, args.test_path, args.output_path, args.name, args.type, hyperparameters)

def train(train_path, val_path, test_path, output_path, name, model_type, hyperparameters):
    ...
    for epoch in range(hyperparameters['epoch_count']):
        for image in dataset_loader:
            ...
            print('epoch [{}/{}], loss:{:.4f}'.format(epoch + 1, hyperparameters['epoch_count'], loss.data[0]))
