...
class SmallCnnWithAutoencoder(BaseModel):
    def __init__(self, kernel_size=3, first_output_channels=8, growth_rate=2, useBatchNorm=False):
        ...
        self._cnn = torch.nn.Sequential(
            ...
            torch.nn.ReLU(True),
            torch.nn.BatchNorm2d(first_output_channels) if useBatchNorm else torch.nn.Sequential(),
            torch.nn.Conv2d(first_output_channels,
                            first_output_channels * growth_rate ** 1,
                            kernel_size, stride=1, padding=conv_padding),
            torch.nn.ReLU(True),
            torch.nn.BatchNorm2d(first_output_channels * growth_rate ** 1) if useBatchNorm else torch.nn.Sequential(),
            torch.nn.MaxPool2d(2, stride=2),
            torch.nn.Conv2d(first_output_channels * growth_rate ** 1,
                            first_output_channels * growth_rate ** 2,
                            kernel_size, stride=1, padding=conv_padding),
            torch.nn.ReLU(True),
            torch.nn.BatchNorm2d(first_output_channels * growth_rate ** 2) if useBatchNorm else torch.nn.Sequential(),
            torch.nn.Conv2d(first_output_channels * growth_rate ** 2,
                            first_output_channels * growth_rate ** 3,
                            kernel_size, stride=1, padding=conv_padding),
            torch.nn.ReLU(True),
            torch.nn.BatchNorm2d(first_output_channels * growth_rate ** 3) if useBatchNorm else torch.nn.Sequential(),
            torch.nn.MaxPool2d(2, stride=2),
            torch.nn.Conv2d(first_output_channels * growth_rate ** 3,
                            first_output_channels * growth_rate ** 4,
                            kernel_size, stride=1, padding=conv_padding),
            torch.nn.ReLU(True),
            torch.nn.BatchNorm2d(first_output_channels * growth_rate ** 4) if useBatchNorm else torch.nn.Sequential(),
            torch.nn.Conv2d(first_output_channels * growth_rate ** 4,
                            first_output_channels * growth_rate ** 5,
                            kernel_size, stride=1, padding=conv_padding),
            torch.nn.ReLU(True),
            torch.nn.BatchNorm2d(first_output_channels * growth_rate ** 5) if useBatchNorm else torch.nn.Sequential(),
